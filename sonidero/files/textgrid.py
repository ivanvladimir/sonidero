...
#
# Modidied by Ivan V. Meza Ruiz (2013)
...
class GenTextGrid:
    def __init__(self,xmin=0,xmax=0):
        self.type="ooTextFile"
        self.xmin=0
        self.xmax=xmax
        self.tiers=[]

    def add_tier(self,name,ttype,xmin=None,xmax=None):
        if not xmin:
            xmin=self.xmin
        if not xmax:
            xmax=self.xmax
    ...
    def update(self,labels):
        for time,mark in labels:
            self.tiers[-1][1].append((time,mark))
        self.tiers[-1][1].sort()


    def prettyprint(self,out):
        print >> out, 'File type = "{0}"'.format(self.type)
        print >> out, 'Object class = "TextGrid"'
        print >> out, ''
        print >> out, 'xmin = {0}'.format(self.xmin)
        print >> out, 'xmax = {0}'.format(self.xmax)
        print >> out, 'tiers? <exists>'
        print >> out, 'size = {0}'.format(len(self.tiers))
        print >> out, 'item []:'
        sep=4
        for i,(tier,points) in enumerate(self.tiers):
            sep=4
            print >> out, ' '*sep,'item [{0}]:'.format(i+1)
            sep=7
            print >> out, ' '*sep,'class = "TextTier"'
            print >> out, ' '*sep,'name = "{0}"'.format(tier['name'])
            print >> out, ' '*sep,'xmin = {0}'.format(tier['xmin'])
            print >> out, ' '*sep,'xmax = {0}'.format(tier['xmax'])
            print >> out, ' '*sep,'points: size = {0}'.format(len(points))
            for ii,point in enumerate(points):
                sep=7
                print >> out, ' '*sep,'points [{0}]:'.format(ii)
                sep=11
                print >> out, ' '*sep,'time = {0}'.format(point[0])
                print >> out, ' '*sep,'mark = "{0}"'.format(point[1])
...
class TextGrid(object):
    ...
    def __init__(self, read_file):
        ...
        self.tiers,self.dtiers = self._find_tiers()
    ...
    def _find_tiers(self):
        ...
        tiers,dtiers = self._load_tiers(header)
        return tiers,dtiers
...
class Tier(object):
    ...
    def __iter__(self):
        for label in self.simple_transcript:
            yield label
    ...
    def next(self):
        if self.idx == (self.size - 1):
            raise StopIteration
        self.idx += 1
        return self.simple_transcript[self.idx]
